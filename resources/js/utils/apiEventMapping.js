...
export default {
    ...
    requestMapping(request, data, screen) {
    ...
    responseMapping(response, data, screen) {
        ...
        task.screen.api_config.forEach((val) => {
            ...
            document.querySelector(`[data-cy="screen-field-${component.config.name}"]`)
                .addEventListener(val.config.event, () => {
                    ...
                    this.requestMapping(val.config.options.request, options, screen);
                    ProcessMaker.apiClient(options)
                        .then(response => {
                            this.responseMapping(val.config.options.response, response, screen);
                        })
                        .catch(error => {
                            console.log(error)
